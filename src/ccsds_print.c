...
void print_packet_summary(const uint8_t *raw, size_t bytes_written, const ccsds_secondary_header_t *sec_header) {
    ...
    printf("Bytes from primary header: %d\n", 6);
    printf("Bytes from secondary and payload: %ld\n", bytes_written - 6);
    

    printf("\n[Decoded Primary Header]\n");
    printf("  Version: %d\n", decoded_primary.version);
    printf("  Type: %d (0=Telemetry, 1=Telecommand)\n", decoded_primary.type);
    ...
    printf("  Packet Length: %d (payload bytes: %d)\n",
    ...
    if (decoded_primary.sec_hdr_flag) {
        ...
        decoded_secondary.type = sec_header->type;

        size_t r = unpack_ccsds_secondary_header(raw, &decoded_secondary);
        printf("\n[Decoded Secondary Header] (unpacked %zu bytes)\n", r);
